...
def multi_validate(valloader, model, criterion, epoch, use_cuda, args):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, name) in enumerate(valloader):
            ...
            # compute output
            outputs, _ = model(inputs[:,:, 0:224,0:224], dropout=False)
            ...
            outputs, _ = model(inputs[:, :, 24:248, 24:248], dropout=False)
